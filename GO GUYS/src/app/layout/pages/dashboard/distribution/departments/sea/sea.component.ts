  ...
  fetchEmployees() {
    this.firestore.collection<Employee>('employees').valueChanges({ idField: 'id' }).subscribe(employeeData => {
      ...
      this.filteredEmployees = employeeData.filter(emp =>
        emp.department?.toLowerCase() === 'sea' && (emp.status?.toLowerCase() ?? 'onboarded') === this.statusFilter
      );

      console.log(`🔵 Filtered SEA Employees (${this.statusFilter}):`, this.filteredEmployees); // ✅ Debugging
