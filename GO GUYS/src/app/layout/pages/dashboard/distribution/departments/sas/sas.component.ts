  ...
  fetchEmployees() {
    this.firestore.collection<Employee>('employees').valueChanges({ idField: 'id' }).subscribe(employeeData => {
      ...
      this.filteredEmployees = employeeData.filter(emp =>
        emp.department?.toLowerCase() === 'sas' && (emp.status?.toLowerCase() ?? 'onboarded') === this.statusFilter
      );

      console.log(`🔵 Filtered SAS Employees (${this.statusFilter}):`, this.filteredEmployees); // ✅ Debugging
