...
@Component({
  ...
  styleUrls: ['./snams.component.scss']
})
export class SnamsComponent implements OnInit {
  ...
  fetchEmployees() {
    this.firestore.collection<Employee>('employees').valueChanges({ idField: 'id' }).subscribe(employeeData => {
      ...
      this.filteredEmployees = employeeData.filter(emp =>
        emp.department?.toLowerCase() === 'snams' && (emp.status?.toLowerCase() ?? 'onboarded') === this.statusFilter
      );

      console.log(`🔵 Filtered SNAMS Employees (${this.statusFilter}):`, this.filteredEmployees); // ✅ Debugging
