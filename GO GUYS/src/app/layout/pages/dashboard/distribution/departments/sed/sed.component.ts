  ...
  fetchEmployees() {
    this.firestore.collection<Employee>('employees').valueChanges({ idField: 'id' }).subscribe(employeeData => {
      ...
      this.filteredEmployees = employeeData.filter(emp =>
        emp.department?.toLowerCase() === 'sed' && (emp.status?.toLowerCase() ?? 'onboarded') === this.statusFilter
      );

      console.log(`🔵 Filtered SED Employees (${this.statusFilter}):`, this.filteredEmployees); // ✅ Debugging
