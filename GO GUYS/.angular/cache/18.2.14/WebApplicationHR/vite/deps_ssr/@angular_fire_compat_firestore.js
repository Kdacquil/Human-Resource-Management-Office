...
import {
  ...
  USE_EMULATOR,
  ɵauthFactory
} from "./chunk-4ZP6VHF6.js";
import "./chunk-XXKSDO26.js";
import "./chunk-TQMK5R3X.js";
import {
  FIREBASE_APP_NAME,
  FIREBASE_OPTIONS,
  ɵcacheInstance,
  ɵfirebaseAppFactory
} from "./chunk-3WFQAYQ5.js";
import {
  firebase
...
import {
  ...
  where
} from "./chunk-7VGQBSR5.js";
import "./chunk-EBS6IGXO.js";
import "./chunk-FUOL32H4.js";
...
// node_modules/@angular/fire/node_modules/@firebase/firestore-compat/dist/node-esm/index.node.esm.js
...
var QueryDocumentSnapshot2 = class extends DocumentSnapshot2 {
  data(options) {
    ...
    if (this._delegate._converter) {
      ...
      debugAssert(data !== void 0, "Document in a QueryDocumentSnapshot should exist");
      return data;
    }
  }
};
...
// node_modules/@angular/fire/fesm2022/angular-fire-compat-firestore.mjs
function _fromRef(ref, scheduler = import_rxjs.asyncScheduler) {
...
function fromDocRef(ref, scheduler) {
  return fromRef(ref, scheduler).pipe((0, import_operators.startWith)(void 0), (0, import_operators.pairwise)(), (0, import_operators.map)((snapshots) => {
...
function fromCollectionRef(ref, scheduler) {
  return fromRef(ref, scheduler).pipe((0, import_operators.map)((payload) => ({
    payload,
    type: "query"
  })));
}
var AngularFirestoreDocument = class {
  ...
  valueChanges(options = {}) {
    return this.snapshotChanges().pipe((0, import_operators.map)(({
      payload
   ...
   */
  get(options) {
    return (0, import_rxjs.from)(this.ref.get(options)).pipe(keepUnstableUntilFirst);
  }
};
function docChanges(query2, scheduler) {
  return fromCollectionRef(query2, scheduler).pipe((0, import_operators.startWith)(void 0), (0, import_operators.pairwise)(), (0, import_operators.map)((actionTuple) => {
...
function sortedChanges(query2, events, scheduler) {
  return docChanges(query2, scheduler).pipe(
    (0, import_operators.scan)((current, changes) => combineChanges(current, changes.map((it) => it.payload), events), []),
    (0, import_operators.distinctUntilChanged)(),
    // cut down on unneed change cycles
    (0, import_operators.map)((changes) => changes.map((c) => ({
      type: c.type,
      payload: c
    })))
  );
...
var AngularFirestoreCollection = class {
  ...
  stateChanges(events) {
    ...
    if (events && events.length > 0) {
      source = source.pipe((0, import_operators.map)((actions) => actions.filter((change) => events.indexOf(change.type) > -1)));
    }
    return source.pipe(
      // We want to filter out empty arrays, but always emit at first, so the developer knows
      // that the collection has been resolve; even if it's empty
      (0, import_operators.startWith)(void 0),
      (0, import_operators.pairwise)(),
      (0, import_operators.filter)(([prior, current]) => current.length > 0 || !prior),
      (0, import_operators.map)(([, current]) => current),
      keepUnstableUntilFirst
    );
  }
  /**
   ...
   */
  auditTrail(events) {
    return this.stateChanges(events).pipe((0, import_operators.scan)((current, action) => [...current, ...action], []));
  }
  /**
  ...
  valueChanges(options = {}) {
    return fromCollectionRef(this.query, this.afs.schedulers.outsideAngular).pipe((0, import_operators.map)((actions) => actions.payload.docs.map((a) => {
      if (options.idField) {
   ...
   */
  get(options) {
    return (0, import_rxjs.from)(this.query.get(options)).pipe(keepUnstableUntilFirst);
  }
  /**
...
var AngularFirestoreCollectionGroup = class {
  ...
  stateChanges(events) {
    ...
    return docChanges(this.query, this.afs.schedulers.outsideAngular).pipe((0, import_operators.map)((actions) => actions.filter((change) => events.indexOf(change.type) > -1)), (0, import_operators.filter)((changes) => changes.length > 0), keepUnstableUntilFirst);
  }
  /**
   * Create a stream of changes as they occur it time. This method is similar to stateChanges()
   * but it collects each event in an array over time.
  ...
  valueChanges(options = {}) {
    ...
    return fromCollectionRefScheduled$.pipe((0, import_operators.map)((actions) => actions.payload.docs.map((a) => {
      if (options.idField) {
    ...
    [this.firestore, this.persistenceEnabled$] = ɵcacheInstance(`${app.name}.firestore`, "AngularFirestore", app.name, () => {
      ...
      if (shouldEnablePersistence && !isPlatformServer(platformId)) {
        const enablePersistence = () => {
          try {
            return (0, import_rxjs.from)(firestore.enablePersistence(persistenceSettings || void 0).then(() => true, () => false));
          } catch (e) {
        ...
        return [firestore, (0, import_rxjs.of)(false)];
      }
...
@firebase/firestore-compat/dist/node-esm/index.node.esm.js:
  (**
   * @license
